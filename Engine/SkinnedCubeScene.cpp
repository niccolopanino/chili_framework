...
SkinnedCubeScene::SkinnedCubeScene(const std::wstring &filename) :
    Scene("Skinned Cube Tex: " + std::string(filename.begin(), filename.end())),
    m_sb_tex(Surface::from_file(filename))
{ }

void SkinnedCubeScene::update(Keyboard & kbd, Mouse & mouse, float dt)
{
...
void SkinnedCubeScene::draw(Graphics &gfx) const
{
    ...
    for (size_t i = 0, end = triangles.m_indices.size() / 3; i < end; i++) {
        ...
        if (!triangles.m_cull_flags[i]) {
            gfx.draw_textured_triangle(triangles.m_vertices[triangles.m_indices[i * 3]],
